# Script for analysing ROX data obtained with R package cellPixels +++++
# Author: Kai Budde
# Created: 2021/07/09
# Last changed: 2021/07/09
...
output_directory <- "ROX"
file <- "ROX_intensity_summary.csv"
...
# ROX red: corrected total fluorescence inside the entire cell
df_data$ROX_red_corrected_total_fluorescence_cell <- NA
df_data$ROX_red_corrected_total_fluorescence_cell <-
  df_data$intensity_sum_red_foreground -
  (df_data$intensity_mean_red_background*df_data$number_of_pixels_foreground)

df_data$ROX_red_corrected_total_fluorescence_cell_per_no_of_nuclei <- NA
df_data$ROX_red_corrected_total_fluorescence_cell_per_no_of_nuclei <-
  df_data$ROX_red_corrected_total_fluorescence_cell /
  df_data$number_of_nuclei


# ROX green: corrected total fluorescence inside the nucleus and inside
# the entire cell
df_data$ROX_green_corrected_total_fluorescence_nucleus <- NA
df_data$ROX_green_corrected_total_fluorescence_nucleus <-
...
df_data$ROX_green_corrected_total_fluorescence_nucleus_per_no_of_nuclei <- NA
df_data$ROX_green_corrected_total_fluorescence_nucleus_per_no_of_nuclei <-
  df_data$ROX_green_corrected_total_fluorescence_nucleus /
  df_data$number_of_nuclei


df_data$ROX_green_corrected_total_fluorescence_cell <- NA
df_data$ROX_green_corrected_total_fluorescence_cell <-
  df_data$intensity_sum_green_foreground -
  (df_data$intensity_mean_green_background*df_data$number_of_pixels_foreground)

df_data$ROX_green_corrected_total_fluorescence_cell_per_no_of_nuclei <- NA
df_data$ROX_green_corrected_total_fluorescence_cell_per_no_of_nuclei <-
  df_data$ROX_green_corrected_total_fluorescence_cell /
  df_data$number_of_nuclei
...
df_green_nuc <- group_by(df_data, magnification, end_time, experiment_number, experiment_group) %>%
  summarise(mean_intensity = mean(ROX_green_corrected_total_fluorescence_nucleus_per_no_of_nuclei))

df_green_nuc_2 <- df_green_nuc %>% tidyr::spread(key = experiment_group, value = mean_intensity)
df_green_nuc_2$stim_over_control <- df_green_nuc_2$Stimulation / df_green_nuc_2$Control
...
plot_green_nuc <- ggplot(df_green_nuc_2, aes(x=end_time,
                                           y=stim_over_control,
  ...
  theme(axis.title.y=element_text(size=18),
        #axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) +
  ylab("Ratio of corrected total fluorescence of ROX greenn\nabove nucleus per # of nuclei (Stim/Control)") +
  xlab("End time")

#print(plot_green_nuc)

ggsave(filename = paste(output_dir, "ROXgreen_nuc.pdf", sep="/"),
       width = 297, height = 210, units = "mm")
ggsave(filename = paste(output_dir, "ROXgreen_nuc.png", sep="/"),
       width = 297, height = 210, units = "mm")
...
ggsave(filename = paste(output_dir, "ROXgreen_cell.pdf", sep="/"),
       width = 297, height = 210, units = "mm")
ggsave(filename = paste(output_dir, "ROXgreen_cell.png", sep="/"),
       width = 297, height = 210, units = "mm")
...
# ROX red in cell in entire image per number of nuclei
plot_red_cell <- ggplot(df_red_cell_2, aes(x=end_time,
  ...
  theme(axis.title.y=element_text(size=18),
        #axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) +
  ylab("Ratio of corrected total fluorescence of ROX redn\nabove cell per # of nuclei (Stim/Control)") +
  xlab("End time")

#print(plot_red_cell)

ggsave(filename = paste(output_dir, "ROXred_cell.pdf", sep="/"),
       width = 297, height = 210, units = "mm")
ggsave(filename = paste(output_dir, "ROXred_cell.png", sep="/"),
       width = 297, height = 210, units = "mm")
